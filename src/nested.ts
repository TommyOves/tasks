...
export function findQuestion(
    ...
    const found_question = questions.find(
        (question: Question): boolean => question.id === id
    );
    if (found_question === undefined) {
        return null;
    } else {
...
export function sumPublishedPoints(questions: Question[]): number {
    const pubQuestions = getPublishedQuestions(questions);
...
export function sameType(questions: Question[]): boolean {
    if (questions.length === 0) {
        ...
        return sameTypeQuestions.length === 0;
    }
}
...
export function duplicateQuestionInArray(
    ...
    dupQuestions.map((question: Question): void => {
        if (question.id === targetId) {
            dupQuestions.splice(
                dupQuestions.indexOf(question) + 1,
                0,
